...
/**
 * Fast CSV Parser using FastCSV library for efficient CSV parsing.
 * This parser loads the entire CSV row at once for maximum performance.
 *
 * @author Claude Code
 * @since Nov 21, 2025
 */
public class CodecCSVParserFast extends CodecParserBaseImpl {
	 ...
	 * @param reader
	 */
	public CodecCSVParserFast(IOContext context, CodecReaderProvider<InputStream> reader) {
		super(null, context, -1, -1, reader.getObjectCodec());
		this.input = reader.getReader();
		try {
			// Use FastCSVParser for efficient CSV parsing
			this.dataMap = FastCSVParser.parse(input);
		} catch (Exception e) {
	 ...
	 * @param is
	 */
	public CodecCSVParserFast(IOContext context, InputStream is) {
	...
	public Version version() {
		return VersionUtil.parseVersion(
				"1.0.0-SNAPSHOT", "org.eclipse.fennec.codec", "codec-csv-fast");
	}
