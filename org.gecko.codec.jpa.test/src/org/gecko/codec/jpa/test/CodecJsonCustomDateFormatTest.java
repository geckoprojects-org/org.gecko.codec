 ...
 */
package org.gecko.codec.jpa.test;
...
@WithFactoryConfiguration(factoryPid = "DefaultCodecFactoryConfigurator", location = "?", name = "test", properties = {
		@Property(key = "type", value="jpa"),
		@Property(key = "genFactory.target", value="(type=jpa)"), 
		@Property(key = "parserFactory.target", value="(type=jpa)")
})
@WithFactoryConfiguration(factoryPid = "DefaultObjectMapperConfigurator", location = "?", name = "test", properties = {
		@Property(key = "codecFactoryConfigurator.target", value="(type=jpa)"),
		@Property(key = "type", value="jpa")
})
@WithFactoryConfiguration(factoryPid = "DefaultCodecModuleConfigurator", location = "?", name = "test", properties = {
		@Property(key = "type", value="jpa")
})
public class CodecJsonCustomDateFormatTest {
	
	@InjectService(cardinality = 0, filter = "("+ EMFNamespaces.EMF_MODEL_NAME + "=person)")
	ServiceAware<ResourceSet> rsAware;
	
	@InjectService(cardinality = 0, filter = "(type=jpa)")
	...
	public void testSerializationDefaultDateFormat() throws InterruptedException, IOException {
		...
		Resource resource = resourceSet.createResource(URI.createURI("test.jpa"));
