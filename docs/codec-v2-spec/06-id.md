...
| `serializeSeparator` | true, false | true | Whether to include separator in STRUCTURED output |
| `separatorKey` | any string | `_separator` | JSON key for the separator field |
...
In STRUCTURED format with multiple ID features, the separator can optionally be included in the JSON output:
...
| ID_ONLY, STRUCTURED | `{"_id": {"_separator": "-", "myId": "John", "mySecondId": "Doe", "myThirdId": "1"}, "test": "..."}` |
